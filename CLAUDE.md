...
- **main ブランチへの直接プッシュは絶対に行わない**（ブランチ保護設定済み）
- すべての変更は必ず Phase ブランチを作成し、PR を通じてマージする
- `git push origin main` は禁止。必ず feature ブランチを push し、PR を作成すること

### ワークフロー

1. `main` から Phase ブランチを作成 (`phase/{番号}-{短い説明}`)
2. 実装 + テスト + ビルド を全て pass させる
3. Phase ブランチをリモートに push し、PR を作成する（description に以下を記述）:
   - 何を実装するか（スコープ）
   - モジュール構成・型設計の方針
   - テスト方針
